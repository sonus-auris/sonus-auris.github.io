...
export function oresWasmLoader({ appId, triggerSelector = DEFAULT_TRIGGER_SELECTOR } = {}) {
  ...
  return {
    ...
    hooks: {
      'astro:config:setup': ({ config, injectScript }) => {
        ...
        const moduleUrl = `${base}owls/marketing-loader.mjs?${query}`;
        injectScript(
          'page',
          `const key = Symbol.for('ores.wasm-loader.marketing.bootstrap.v1');
